 ...
 * 操作状态变量及其协方差的帮助类
 * 这个类拥有基于EKF系统的所有核心逻辑
 * 它具有更改协方差以及从状态中添加和删除元素的所有函数
 * 这里的所有函数都是静态的，因此是自包含的，以便将来可以跟踪和更新多个状态
 * 建议直接查看这个类的代码，以清楚地了解在每个页面和匹配的文档页面中到底在做什么
 */
class StateHelper {
...
public:
   ...
   * 这将固定初始协方差矩阵仪表自由度（4dof,偏航和位置）
   * A VIO system has 4dof unobservabile directions which can be arbitarily picked.
   * 一个VIO系统具有能够自由选取的4个自由度的不可观测方向，这意味着在启动时我们可以将偏航和位置固定为100%已知
   * 因此，在初始化后确定从全局到当前IMU的方向后，我们可以将全局误差传播到新的IMU位姿中，在这种情况下位置直接相等，但方向需要传播
