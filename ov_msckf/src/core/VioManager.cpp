...
void VioManager::feed_measurement_imu(const ov_core::ImuData &message) {
  ...
  // 如果我们至少有一个IMU测量值大于相机时间，我们就能进行处理
  // 相机时间戳小于校正后的imu时间戳
...
bool VioManager::try_to_initialize() {
  ...
  // 如果我们没有启用静止更新我们将等待一个猛拉，否则我们可以立即初始化，因为零速更新将处理静止情况
  ...
  state->_imu->set_fej(imu_val);//将状态变量存入到第一估计雅克比矩阵变量中
  state->_timestamp = time0;
  startup_time = time0;
  // 上面是将初始化后的imu的q,p,v,bg,ba存入到状态变量中
