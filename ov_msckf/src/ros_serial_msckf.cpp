...
int main(int argc, char **argv) {
  ...
  nh.param<std::string>("path_bag", path_to_bag, "/home/pogman/dataset/MH_01_easy.bag");
  ...
  time_init += ros::Duration(bag_start);//返回开始运行包的时间
  ros::Time time_finish = (bag_durr < 0) ? view_full.getEndTime() : time_init + ros::Duration(bag_durr);
  //当bag_durr<0的时候得到前一个的值，否则得到后面的值
  ...
  //auto关键字 一是在变量声明时根据初始化表达式自动推断该变量的类型，二是在声明函数时作为函数返回值的占位符
  ...
  for (const auto &topic : topic_cameras) {
    auto view_tmp = std::make_shared<rosbag::View>(bag, rosbag::TopicQuery(topic.second), time_init, time_finish);//得到第二个相机的主题
    view_cameras.push_back(view_tmp);//用于存储相机主题
    view_cameras_iterators.push_back(view_tmp->begin());
  }

  // Record the current measurement timestamps 记录最近测量时间戳（第一个IMU和相机测量的时间戳以及下一个测量的时间戳）
  ...
  while (ros::ok()) {
    ...
    // 如果当前所有相机数据的时间都大于IMU，根据第一个测量数据得到
    ...
    if (should_process_imu) {
      ...
      // 转换数据格式，即将从ros话题接受到的消息存入自定义IMU数据中
    ...
    if (params.state_options.num_cameras == 2) {
      ...
      while (!have_found_pair && view_cameras_iterators.at(0) != view_cameras.at(0)->end() &&
        ...
        // 如果当前左图和右图比下一个图像更接近，进行匹配
        // 当图相对中有一个图像丢失时，通过时间距离匹配去掉相应的图像
