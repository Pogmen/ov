...
bool InertialInitializer::initialize_with_imu(double &time0, Eigen::Matrix<double, 4, 1> &q_GtoI0, Eigen::Matrix<double, 3, 1> &b_w0,
  ...
  // 我们还应该检查旧状态是否低于阈值，这种情况出现于开始运动的时候，因此我们需要等待一段时间的静止运动
  ...
  // 如果高于阈值我们不用等待震动，如果我们不是静止的，我们应该等到静止
  ...
  // Ro是从全局坐标系到IMU坐标系的旋转矩阵，即IMU基坐标在全局坐标系中的位置
  ...
  // 将我们的偏差设置为噪声（从加速度计偏差中减去重力）
  // 加速度计的偏置即是旧窗口中加速度的平均值
